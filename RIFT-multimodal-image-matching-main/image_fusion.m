function image_fusion(image_1,image_2,solution,n,flag)

[M1,N1,num1]=size(image_1);
[M2,N2,num2]=size(image_2);
%若两幅图像都是 RGB，则融合图像尺寸为 3*M1 x 3*N1
%若其中一幅图像是灰度图，则将其转换为灰度，并相应调整融合图像的维度
if(num1==3 && num2==3)
...
same_index = find((f_1 & f_2)); 
index_1 = find(f_1 & ~f_2);
index_2 = find(~f_1 & f_2);

fusion_image(same_index)=f_1(same_index) +f_2(same_index);%二值化的时候采用去掉白色背景用
%fusion_image(same_index)=f_1(same_index)./2 + f_2(same_index);
...
x = sprintf('%04d',n);
str1 = ['.\new_event\', 'frame_', x, '.png'];
% 保存裁剪后的事件图像
if (flag == 1)
    imwrite(uint8(f_2), str1, 'png');  % 将裁剪后的事件图像保存

if(flag==2)
    str=['.\mask\','frame_',x,'.png'];
else
    str=['.\fusion_result\','frame_',x,'.png'];
end
imwrite(fusion_image,str,'png');
